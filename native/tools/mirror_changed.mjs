 ...
 *
 * • התאריכים — `updatedAt`, `originalDate`, `releasedAt`. אמורים לתאר
 *   מתי התוסף עודכן, אבל `/api/plugins` מחזיר בהם את **היום**: נמדד
 *   ב-2026-09-21, וכל 39 התוספים חזרו עם `updatedAt: "2026-09-21"`.
 *   בלי הנטרול הקטלוג נבדל בכל יום, והג'וב פרסם ‎109MB מדי יום — עשר
 *   ריצות מוצלחות רצופות, כולן פרסמו.
 *
 *   ⚠️ המחיר היה כפול, והחלק השני הוא הגרוע: קובץ הרצה לא חתום שה-hash
 *   שלו מתחלף כל יום לעולם אינו צובר מוניטין הורדות, וכל עותק יומי הוא
 *   הגרלה מחדש מול הענן של Defender. אחת מהן יצאה
 *   `Trojan:Win32/Wacatac.B!ml`, ו-Chrome מחק את החבילה אצל המשתמשים.
 *
 * המחיר מדוד ומקובל: החבילה נושאת מונים ותאריכים מהיום שבו נארזה. הם
 * מתרעננים בסנכרון אצל מי שיש לו רשת, ואצל המנותק הם ממילא לידיעה בלבד.
 *
 * ⚠️ עדכון אמיתי של תוסף אינו מסתתר כאן: הוא משנה את `version` ואת
 * רשימת הקבצים וגדליהם, ושניהם מושווים בנפרד.
 */
const VOLATILE_KEYS = new Set([
  'lastSync', 'downloadCount',
  'ratingAvg', 'ratingCount', 'ratingVerifiedCount', 'ratingBreakdown',
  'updatedAt', 'originalDate', 'releasedAt',
]);
